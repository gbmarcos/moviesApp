...
void main() {
  ...
  test('map', () {
    ...
    expect(
      const AsyncValue<int>.loading(
        ...
        error: (_) => throw Error(),
        initial: (_) => throw Error(),
        loading: (AsyncLoading<int> loading) => 'loading ${loading.previous?.valueOrNull}',
      ),
      'loading 42',
    );
  });
  ...
  test('when', () {
    ...
    expect(
      const AsyncValue<int>.loading(
        ...
        error: (a, b) => throw Error(),
        initial: (_) => throw Error(),
        loading: (previous) => 'loading ${previous?.valueOrNull}',
      ),
      'loading 42',
    );
  });
  ...
  test('AsyncValue.asData', () {
    expect(
      const AsyncValue.data(42).getOrNull,
      const AsyncData<int>(42),
    );
    expect(const AsyncValue<void>.loading().getOrNull, isNull);
    expect(AsyncValue<void>.error(Error()).getOrNull, isNull);
  });

  test('AsyncValue.value', () {
    expect(const AsyncValue.data(42).valueOrNull, 42);
    expect(
      const AsyncValue<void>.loading().getOrNull,
      isNull,
    );

    final error = Error();
    expect(
      AsyncValue<void>.error(error).getOrNull,
      isNull,
    );
  });
  ...
  test('AsyncValue.data handles null', () {
    expect(
      const AsyncValue<int?>.data(null).getOrNull,
  ...
  test('AsyncValue.guard emits the data when the created future completes', () async {
    await expectLater(
  ...
  test('AsyncValue.guard emits the error when the created future fails', () async {
    final stack = StackTrace.current;
...
class CustomError<T> extends AsyncError<T> {
  CustomError(Object error, [StackTrace? stackTrace]) : super(error, stackTrace);
}
